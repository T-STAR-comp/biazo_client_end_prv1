...
export function LoadingScreen({ label = "Loading", compact = false }: LoadingScreenProps) {
  return (
    <div
      className={`flex flex-col items-center justify-center ${compact ? "py-16" : "min-h-[min(420px,60vh)] py-24"}`}
      role="status"
      aria-live="polite"
      aria-label={label}
    >
      ...
      <p className="mt-6 text-sm font-medium tracking-wide text-muted-foreground">{label}</p>
...
export function LoadingOverlay({ label = "Please wait" }: { label?: string }) {
  return (
    <div className="absolute inset-0 z-20 flex items-center justify-center rounded-[inherit] bg-background/75 backdrop-blur-sm">
